...
const StatusIcon = ({ status }: { status: MessageStatus }) => {
  const configs = {
    ...
    sent: { color: "text-blue-500", label: "Sent", icon: <svg className="h-2 w-2 sm:h-3 sm:w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg> },
    delivered: { color: "text-green-500", label: "Delivered", icon: <svg className="h-2 w-2 sm:h-3 sm:w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    error: { color: "text-red-500", label: "Failed", icon: <svg className="h-2 w-2 sm:h-3 sm:w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
  };
...
const ReactionPicker = ({ reactions, userId, onReact }: { reactions?: Reaction[]; userId: string; onReact: (type: ReactionType) => void }) => (
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border dark:border-gray-700 p-0.5 sm:p-1 flex gap-0 z-20">
    {CONFIG.REACTIONS.map((reaction) => {
      const isActive = utils.sanitizeReactions(reactions || []).some((r) => r.userId === userId && r.type === reaction);
      return (
        <button key={reaction} onClick={() => onReact(reaction)} className={`hover:bg-gray-100 dark:hover:bg-gray-700 p-0.5 sm:p-1 rounded transition-colors text-xs sm:text-base ${isActive ? "bg-blue-100 dark:bg-blue-900" : ""}`}>
          {reaction}
        </button>
      );
    })}
  ...
  return (
    <div className="flex flex-wrap gap-0.5 justify-end">
      {unique.map((reaction, idx) => {
        const isActive = utils.sanitizeReactions(reactions || []).some((r) => r.userId === userId && r.type === reaction.type);
        return (
          <div key={idx} className={`inline-flex items-center gap-0.5 bg-white dark:bg-gray-800 border rounded-full px-[2px] py-[1px] sm:px-1 sm:py-0.5 text-[8px] sm:text-xs shadow-md ${isActive ? "border-blue-500 bg-blue-50 dark:bg-blue-900/50" : "border-gray-300 dark:border-gray-600"}`}>
      ...
      <div className={`relative max-w-[85%] sm:max-w-[70%] md:max-w-[60%] min-w-[40px] ${isOwn ? 'mr-2' : 'ml-2'}`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        {isHovered && (
          <div className={`absolute -top-8 ${isOwn ? "right-0" : "left-0"} z-10`}>
            <ReactionPicker reactions={message.reactions} userId={currentUserId} onReact={onReact} />
          </div>
        )}
        <div className={`rounded-2xl p-2.5 sm:p-3 ${isOwn ? "bg-blue-500 text-white shadow-md" : "bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 shadow-sm"} overflow-hidden`}>
              ...
              {message.imageThumbnail && !imageLoaded && (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-600 rounded-lg">
                  <div className="animate-pulse w-full h-full flex items-center justify-center">
                    <img src={message.imageThumbnail} alt="Loading thumbnail" className="max-w-full max-h-[200px] rounded-lg blur-sm" style={{ maxWidth: '100%', height: 'auto' }} />
                  </div>
                </div>
              )}
              <img src={message.imageUrl} alt="Shared image" className="max-w-full max-h-[300px] rounded-lg cursor-pointer" onClick={() => window.open(message.imageUrl, '_blank')} onLoad={() => setImageLoaded(true)} style={{ maxWidth: '100%', height: 'auto' }} />
            </div>
          ) : (
            <p className="break-words whitespace-pre-wrap text-sm sm:text-base overflow-hidden">
              {utils.highlightMentions(message.text, currentUserId, onlineUsers)}
            </p>
          )}
          {isOwn && message.status && (
            <div className="mt-1 flex justify-end"><StatusIcon status={message.status} /></div>
          )}
        </div>
        {hasReactions && (
          <div className={`absolute -bottom-4 ${isOwn ? "right-0" : "left-0"} z-5`}>
            <div className="translate-y-2"><ReactionDisplay reactions={message.reactions} userId={currentUserId} /></div>
          </div>
        )}
      </div>
    </div>
...
const UserListItem = ({ user, isCurrentUser }: { user: User; isCurrentUser: boolean }) => (
  <div className={`flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all cursor-pointer ${isCurrentUser ? "bg-blue-50 dark:bg-blue-900/50" : ""}`}>
    <div className="relative flex-shrink-0">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-semibold shadow-md ${isCurrentUser ? "bg-green-500" : "bg-blue-500"}`}>
    ...
    <div className="flex-1 min-w-0">
      <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">
        {user.username}
        {isCurrentUser && <span className="ml-2 text-xs text-green-600 dark:text-green-400 font-normal">(You)</span>}
      </p>
...
const JoinScreen = ({ username, onUsernameChange, onSubmit, theme, toggleTheme }: { username: string; onUsernameChange: (value: string) => void; onSubmit: (e: React.FormEvent) => void; theme: Theme; toggleTheme: () => void }) => (
  <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center p-4 transition-colors duration-300">
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 sm:p-10 max-w-md w-full transition-colors duration-300 relative">
      <button onClick={toggleTheme} className="absolute top-4 right-4 p-2 rounded-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
        {theme === "light" ? (
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">Welcome to Chatto</h2>
        <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm">Connect with friends in real-time</p>
      </div>
      <form onSubmit={onSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Username</label>
          <input type="text" value={username} onChange={(e) => onUsernameChange(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm sm:text-base bg-white dark:bg-gray-700 text-gray-900 dark:text-white" placeholder="Enter your username" required maxLength={20} autoFocus />
        </div>
        <button type="submit" className="w-full bg-blue-500 text-white py-3 rounded-xl hover:bg-blue-600 transition-all duration-200 font-semibold text-sm sm:text-base shadow-lg hover:shadow-xl">Join Chat</button>
      </form>
    </div>
  </div>
);
  ...
  <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-hidden transition-colors duration-300">
    ...
    <div className="bg-white dark:bg-gray-800 shadow-lg border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
      <div className="px-4 sm:px-6 py-3 flex justify-between items-center w-full lg:max-w-[90%] xl:max-w-[80%] mx-auto">
        <div className="flex items-center gap-3">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl">
            <svg className="h-5 w-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <div className="w-10 h-10 bg-blue-500 rounded-xl flex items-center justify-center shadow-md">
            <NextImage src="/next.svg" alt="Logo" width={24} height={6} className="brightness-0 invert" />
          </div>
          <div><h1 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-white">Chatto</h1><p className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">Gawa ni Jirik</p></div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-full"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><span className="text-sm text-gray-600 dark:text-gray-300">{username}</span></div>
    ...
    <div className="flex-1 flex items-center justify-center p-4 overflow-hidden">
      <div className="w-full lg:max-w-[90%] xl:max-w-[80%] h-full flex gap-4">
        ...
        <div className={`lg:flex lg:w-72 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex-shrink-0 flex flex-col overflow-hidden ${isMobileMenuOpen ? 'fixed inset-y-0 left-0 z-50 w-72' : 'hidden lg:flex'}`}>
          <div className="p-4 bg-gradient-to-r from-blue-500 to-indigo-600">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center"><svg className="h-4 w-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg></div>
          ...
          <div className="flex-1 overflow-y-auto">
            {onlineUsers.length === 0 ? (
              <div className="text-center text-gray-500 dark:text-gray-400 py-12"><p className="text-sm">No active users</p></div>
            ) : (
              onlineUsers.map((user: User) => <UserListItem key={user.id} user={user} isCurrentUser={user.id === userId} />)
            )}
          </div>
        </div>
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex flex-col overflow-hidden relative">
          {showLoadMoreButton && hasMoreMessages && !isLoading && messages.length > 0 && (
            <div className="sticky top-0 z-10 p-3 flex justify-center bg-white/95 dark:bg-gray-800/95 border-b flex-shrink-0">
              <button onClick={onLoadMoreMessages} disabled={isLoadingMore} className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-xl text-sm flex items-center gap-2 shadow-md">
                {isLoadingMore ? <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg><span>Loading older messages...</span></> : <><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg><span>Load older messages</span></>}
              </button>
            </div>
          )}
          ...
          <div ref={messagesContainerRef} onScroll={onScroll} className="flex-1 overflow-y-auto p-4 space-y-3">
            {isLoading ? (
              ...
              <div className="flex justify-center items-center h-full"><div className="text-center"><svg className="h-16 w-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg><p className="text-gray-500 dark:text-gray-400 text-lg">No messages yet</p><p className="text-gray-400 dark:text-gray-500 text-sm">Start the conversation!</p></div></div>
            ) : (
              messages.map((message: Message) => (
                <MessageBubble key={message.id} message={message} currentUserId={userId} isHovered={hoveredMessageId === message.id} onMouseEnter={() => setHoveredMessageId(message.id)} onMouseLeave={() => setTimeout(() => setHoveredMessageId(null), 200)} onReact={(type) => onAddReaction(message.id, type)} onlineUsers={onlineUsers} />
              ))
            )}
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800 relative">
            ...
            <form onSubmit={onSendMessage} className="space-y-2">
              <div className="flex gap-2">
                ...
                <button type="submit" className="bg-blue-500 text-white px-5 py-2 rounded-xl hover:bg-blue-600 font-medium text-sm shadow-md">Send</button>
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400 px-1">
                💡 Tip: Type <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1 rounded">@</span> to mention someone — suggestions will appear above
              </div>
            </form>
          </div>
        </div>
...
export default function Home() {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (isNearTop && hasMoreMessages && !isLoadingMore && messages.length > 0) setShowLoadMoreButton(true);
