...
const CONFIG = {
  ...
  CACHE_LOADING_ANIMATION_DURATION: 1500, // Duration for loading animation
};
...
const utils = {
  ...
  saveMessagesToCache: (messages: Message[]) => {
    try {
      // Only cache the last 50 messages for quick restore
...
const LoadingNewMessages = () => (
  <div className="flex justify-center my-4">
    <div className="bg-blue-50 dark:bg-blue-900/30 rounded-full px-4 py-2 flex items-center gap-2 shadow-sm border border-blue-200 dark:border-blue-800 animate-pulse">
      <svg
...
const useMessages = () => {
  ...
  const loadMessages = useCallback(async () => {
    ...
    const cachedMessages = utils.loadMessagesFromCache();
    if (cachedMessages && cachedMessages.length > 0) {
      setMessages(cachedMessages);
      setHasCachedData(true);
      // Show loading animation while fetching fresh data
      setIsLoadingNewMessages(true);
      // Scroll to bottom to show latest cached messages
      setTimeout(() => {
    ...
    try {
      ...
      // Check if there are new messages beyond the cached ones
      const hasNewMessages = cachedMessages && 
        freshMessages.length > cachedMessages.length;
      
      if (hasNewMessages) {
        // Show loading animation for smooth transition
        await new Promise(resolve => setTimeout(resolve, CONFIG.CACHE_LOADING_ANIMATION_DURATION));
      }
      ...
      // Scroll to bottom to show new messages if needed
      if (hasNewMessages) {
        setTimeout(() => {
  ...
  // Save messages to cache whenever they change (for new messages via Pusher)
  useEffect(() => {
    if (!isLoading && messages.length > 0 && !hasCachedData) {
      utils.saveMessagesToCache(messages);
    }
  }, [messages, isLoading, hasCachedData]);
      ...
      <div className="w-full lg:max-w-[90%] xl:max-w-[80%] h-full flex gap-4">
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex flex-col overflow-hidden relative">
          ...
          <div ref={messagesContainerRef} onScroll={onScroll} className="flex-1 overflow-y-auto p-4">
                ...
                {messages.map((message: Message, index: number) => (
                  <div 
                    ...
                    className={`transition-all duration-500 ease-in-out ${
                      isLoadingNewMessages ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'
                    }`}
                    style={{
                      animation: isLoadingNewMessages 
                        ? `slideInFromBottom 0.5s ease-out forwards ${index * 30}ms`
                        : 'none'
                    }}
                  >
                    {firstUnreadId === message.id && newMessageCount > 0 && (
                      <UnreadDivider count={newMessageCount} />
                    )}
                    <MessageBubble
                      message={message}
            ...
            {isLoadingNewMessages && <LoadingNewMessages />}
