  ...
  highlightMentions: (
    ...
    while ((match = mentionRegex.exec(text)) !== null) {
      ...
      const isMentioningMe =
        !isEveryone && currentUsername !== "" && mentionedLower === currentUsername;
      ...
      parts.push(
        <span
          ...
          onClick={() => {
            if (userExists && !isMentioningMe && !isEveryone) {
...
const useMessages = () => {
        ...
        for (const [id, msg] of Object.entries(messagesData)) {
          ...
          if (messageData?.text && messageData?.username) {
            const message: Message = {
              ...
              reactions: utils.sanitizeReactions(
  ...
  const loadMessages = useCallback(async () => {
    setIsLoading(true);
    try {
      const loadedMessages = await loadAndCombineMessages(
...
// Pusher Hook with mention sound
const usePusher = (
  isJoined: boolean,
  setMessages: React.Dispatch<React.SetStateAction<Message[]>>,
  ...
  const soundPlayDebounceMs = 1000;
  ...
  useEffect(() => {
    ...
    channel.bind("new-message", async (data: any) => {
      ...
      if (isMentioned) {
        const now = Date.now();
        if (now - lastMentionTimeRef.current >= soundPlayDebounceMs) {
  ...
  }, [isJoined, setMessages, setNewMessageCount, isUserScrolled, messagesEndRef, currentUserId, currentUsername]);
};
  ...
  <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-hidden transition-colors duration-300">
    ...
    <div className="flex-1 flex items-center justify-center p-4 overflow-hidden">
      <div className="w-full lg:max-w-[90%] xl:max-w-[80%] h-full flex gap-4">
        {/* Sidebar */}
        <div
          className={`lg:flex lg:w-72 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex-shrink-0 flex flex-col overflow-hidden ${
            isMobileMenuOpen
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex flex-col overflow-hidden relative">
          {showLoadMoreButton && hasMoreMessages && !isLoading && messages.length > 0 && (
            <div className="sticky top-0 z-10 p-3 flex justify-center bg-white/95 dark:bg-gray-800/95 border-b flex-shrink-0">
              <button
                ...
                className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-xl text-sm flex items-center gap-2 shadow-md"
              >
                {isLoadingMore ? (
                  <>
                    <svg
          ...
          {showScrollButton && newMessageCount === 0 && (
            <button
              ...
              className="absolute bottom-20 right-4 bg-blue-500 text-white rounded-full p-2 shadow-lg hover:bg-blue-600 z-10"
            >
              <svg
                className="h-5 w-5"
                fill="none"
          ...
          {newMessageCount > 0 && (
            <button
              onClick={onScrollToBottom}
              className="absolute bottom-20 right-4 bg-blue-500 text-white rounded-full px-3 py-2 shadow-lg hover:bg-blue-600 z-10 text-sm flex items-center gap-2"
            >
              <svg
                className="h-4 w-4"
                fill="none"
            ...
            {isLoading ? (
              ...
              messages.map((message: Message) => (
                <MessageBubble
                  ...
                  onMouseLeave={() =>
                    setTimeout(() => setHoveredMessageId(null), 200)
                  }
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800 relative">
            ...
            <form onSubmit={onSendMessage} className="space-y-2">
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="bg-blue-500 text-white px-5 py-2 rounded-xl hover:bg-blue-600 font-medium text-sm shadow-md"
                >
                  Send
                </button>
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400 px-1">
                ...
                to mention someone — suggestions will appear above
              </div>
            </form>
          </div>
        </div>
...
export default function Home() {
  ...
  // Persistent userId - same across reloads
  ...
  const { onlineUsers, userId, usernameRef, updateLastActive, loadOnlineUsers } =
  ...
  const sendMessage = useCallback(
    async (e: React.FormEvent) => {
      ...
      const messageId = utils.generateId();
      const { mentions } = utils.parseMentions(
        messageText,
        onlineUsers,
        userId
      );
      ...
      // Send message asynchronously - don't await here
